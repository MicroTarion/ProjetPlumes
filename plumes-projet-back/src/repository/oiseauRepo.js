...
module.exports = class oiseauRepository {
    ...
    async getAllOiseaux() {
        return await con.promise().query("SELECT o.nom AS NomOiseau, o.illustration AS IllustrationOiseau, ord.nom AS NomOrdre, f.nom AS NomFamille, p.illustration AS PhotoPlume FROM oiseaux o INNER JOIN posseder pos ON o.Id_Oiseaux = pos.Id_Oiseaux INNER JOIN plumes p ON pos.Id_Plumes = p.Id_Plumes INNER JOIN famille f ON o.Id_Famille = f.Id_Famille INNER JOIN ordre ord ON o.Id_Ordre = ord.Id_Ordre;").then((result) => {
